...
async function main() {
  ...
  const filePath = path.join(process.cwd(), "public/media/nvco-portfolio.jpg")
  if (!existsSync(filePath)) throw new Error(`Missing ${filePath}`)

  const created = await payload.create({
    collection: "media",
    data: { alt: "NVCo website mockup" },
    filePath,
    overrideAccess: true,
  })
  ...
  const portfolio = await payload.find({
    ...
    overrideAccess: true,
    draft: true as any,
    where: { slug: { equals: "nvco" } },
  })

  const doc = portfolio.docs[0]
  if (!doc) throw new Error("NVCo portfolio item not found")
  ...
  for (const locale of ["en", "fa", "ar"] as const) {
    ...
    console.log(`Published nvco (${locale}) -> media #${mediaId}`)
  }
  ...
  console.log("VERIFY nvco:", image?.id, image?.filename)
}
