    ...
    <main className="container mx-auto px-4 py-16 md:py-24" dir={rtl ? "rtl" : "ltr"}>
      <RevealOnScroll>
        <div className="relative overflow-hidden rounded-3xl border bg-card/60 p-6 shadow-sm backdrop-blur md:p-10">
          ...
          <p className="mb-3 text-sm font-medium text-primary">{terms.lastUpdated}</p>
          <h1 className="max-w-3xl text-3xl font-bold tracking-tight sm:text-4xl">{terms.title}</h1>
          <p className="mt-4 max-w-3xl text-muted-foreground">{terms.subtitle}</p>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-muted-foreground">{terms.intro}</p>
        </div>
      </RevealOnScroll>
      ...
      <RevealOnScroll className="mt-10">
        <section className="relative overflow-hidden rounded-2xl border bg-card/70 p-6 shadow-sm backdrop-blur">
          ...
          <h2 className="text-xl font-semibold">{terms.contactCta.title}</h2>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-muted-foreground">{terms.contactCta.body}</p>
          <Button asChild className="mt-5">
            <Link href={`/${params.locale}/contact`}>{terms.contactCta.button}</Link>
          </Button>
        </section>
      </RevealOnScroll>
    </main>
